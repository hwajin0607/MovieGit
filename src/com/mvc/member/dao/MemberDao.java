...
public class MemberDao {
	...
	public ArrayList<MovieDto> like(String uIdx,int page1) throws SQLException {
		...
		String sql = "select distinct f.rnum, f.mfidx, m.midx, f.mfnew from (select ROW_NUMBER() OVER(ORDER BY f.mfidx DESC) AS rnum, m.midx, f.mfidx, f.mfnew from moviefoster f, movie m WHERE f.midx=m.midx AND m.midx IN(select m.midx from movie m, userGenre u WHERE m.mgenre=u.ggenre AND u.uidx=?)) f, movie m where f.midx = m.midx and f.rnum BETWEEN ? AND ?  ORDER BY f.rnum";
		ps =conn.prepareStatement(sql);
		ps.setString(1, uIdx);
		ps.setInt(2, start);
        ps.setInt(3, end);
